...
# 
# # Fit model
# for_train = agg_data[, c("pickups", "hourID", "clusterID", "day", "is_holiday", "mean_temp_F")]
# fit = lm(pickups ~ .
#          , data = for_train)
# # Reduce model size
# clean_fit = fit
# clean_fit$fitted.values <- clean_fit$qr$qr <- clean_fit$residuals <- clean_fit$model <- clean_fit$effects <- NULL
# 
...
# 
# save(clean_fit, loc_zone_dim, hour_dim
#      , file = "demo_model_files.RData")

load("weather.RData")
load("demo_model_files.RData")
...
pred_pickups = function(hour, date, location_name) {
  ...
  new = data.frame(hourID = as.factor(hourID)
                   , clusterID = as.factor(clusterID)
                   , day = as.factor(day)
                   , is_holiday = is_holiday
                   , mean_temp_F = mean_temp_F)
  
  pred = predict(clean_fit, newdata = new)
